...
var ArcGisCatalogGroup = function(terria) {
     ...
     * grouped for flattened. Ty Default it is not flattened. If groupBy === 'none', then it is flattened. This property is observable.
     * @type {String}
     */
    this.groupBy = undefined;

    knockout.track(this, ['url', 'dataCustodian', 'blacklist', 'itemProperties', 'groupBy']);
};
...
function addLayersRecursively(mapServiceGroup, topLevelJson, topLevelLayersJson, parentID, layers, items, dataCustodian) {
    ...
    for (var i = 0; i < layers.length; ++i) {
        ...
        if (layer.type === 'Group Layer') {
            ...
            if(mapServiceGroup.groupBy !== "none"){
                group = new CatalogGroup(mapServiceGroup.terria);
            ...
            if (defined(layer.Name) && layer.Name.length > 0) {
                var all = createDataSource(mapServiceGroup, topLevelJson, topLevelLayersJson, layer, dataCustodian );
                if(mapServiceGroup.groupBy !== "none"){
         ...
         addLayersRecursively(mapServiceGroup, topLevelJson, topLevelLayersJson, layer.id, layers, recurseItems, dataCustodian);
